...
// sdk_ack_callback
//----------------------------------------------------------
// only for test default ack func.
...
// additional mode test
// tset mode 10: vert position, hori velocity, yaw angular rate
void basic_test_mode10(bool &is_init)
{
	static int cnt;
	if(!is_init)
	{
	...
	else
	{
		...
		App_Send_Data(0, 0, MY_CTRL_CMD_SET,API_CTRL_REQUEST, (uint8_t*)&send_data, sizeof(send_data), NULL, 0, 0);
		printf("alti %f /n height %f \n",recv_sdk_std_msgs.pos.alti, recv_sdk_std_msgs.pos.height);
	}
}

void basic_test_gimbal_angle(bool &is_init)
{
	static int cnt;
	if(!is_init)
	{
	...
	else
	{
		...
		if(cnt <= 1)
		{
            send_data.yaw_angle   = 0;  // unit 0.1 degree
			send_data.roll_angle  = 0;
			send_data.pitch_angle = -800;
			send_data.ctrl_byte.base = 1;
			send_data.duration = 10; //unit 0.1 s 
			App_Send_Data(0, 0, MY_CTRL_CMD_SET,API_CTRL_GIMBAL_ANGLE, (uint8_t*)&send_data, sizeof(send_data), NULL, 0, 0);
			printf("gimbal yaw %d roll %d pitch %d base %d\n", send_data.yaw_angle, send_data.roll_angle, send_data.pitch_angle, send_data.ctrl_byte.base);
		}
		else if(cnt < 6*50)
		{
			// waitting
...
void basic_test_gimbal_speed(bool &is_init)
{
	static int cnt;
	if(!is_init)
	{
	...
	else
	{
		...
		if(cnt < 10*50)
		{
            send_data.yaw_anglar_rate  = 45;
            send_data.roll_anglar_rate  = 45;
            send_data.pitch_anglar_rate = 45;
            send_data.ctrl_byte.ctrl_switch = 1;
            App_Send_Data(0, 0, MY_CTRL_CMD_SET, API_CTRL_GIMBAL_SPEED, (uint8_t*)&send_data, sizeof(send_data), NULL, 0, 0);
            printf("gimbal yawr %d rollr %d pitchr %d \n", send_data.yaw_anglar_rate, send_data.roll_anglar_rate, send_data.pitch_anglar_rate);
		}
		else if(cnt < 20*50)
		{
            send_data.yaw_anglar_rate  = -45;
...
// test HP
void basic_test_hp_start_ack_callback(ProHeader *header)
{
	uint16_t ack_data;
	printf("Sdk_ack_cmd0_callback,sequence_number=%d,session_id=%d,data_len=%d\n", header->sequence_number, header->session_id, header->length - EXC_DATA_SIZE);
	memcpy((uint8_t *)&ack_data,(uint8_t *)&header->magic, (header->length - EXC_DATA_SIZE));
	...
	else
	{
		//char result[6][50]={{"REQ_TIME_OUT"},{"REQ_REFUSE"},{"CMD_RECIEVE"},{"STATUS_CMD_EXECUTING"},{"STATUS_CMD_EXE_FAIL"},{"STATUS_CMD_EXE_SUCCESS"}};
		//printf("random_test Cmd result: %s \n", *(result+ack_data));
		printf("[HP]start result: %04X \n", ack_data);
	}
}
void basic_test_hp_start(void)
{
	cmd_mission_hp_request_start send_data = {0};
	send_data.setting.hp_alti	= 10.0f;
	send_data.setting.hp_lati	= 0.39447438752;
	send_data.setting.hp_lonti	= 1.98715607943;
	send_data.setting.hp_radius	= 10.0f;
	
	send_data.setting.speed_level	= 15;
	
	send_data.setting.is_clockwise	= 1;
	send_data.setting.start_point	= 3;
	send_data.setting.yaw_mode	= 0;

	App_Send_Data(2, 1, MY_CTRL_CMD_SET, API_MISSION_HP_START, (uint8_t*)&send_data, sizeof(send_data), basic_test_hp_start_ack_callback, 1000, 0);
	printf("[HP] send HP start msg %f %f %f %f  \n", send_data.setting.hp_alti, send_data.setting.hp_lati,send_data.setting.hp_lonti, send_data.setting.hp_radius);	
}

void basic_test_hp_stop_ack_callback(ProHeader *header)
{
	uint16_t ack_data;
	printf("Sdk_ack_cmd0_callback,sequence_number=%d,session_id=%d,data_len=%d\n", header->sequence_number, header->session_id, header->length - EXC_DATA_SIZE);
	memcpy((uint8_t *)&ack_data,(uint8_t *)&header->magic, (header->length - EXC_DATA_SIZE));
	...
	else
	{
		//char result[6][50]={{"REQ_TIME_OUT"},{"REQ_REFUSE"},{"CMD_RECIEVE"},{"STATUS_CMD_EXECUTING"},{"STATUS_CMD_EXE_FAIL"},{"STATUS_CMD_EXE_SUCCESS"}};
		//printf("random_test Cmd result: %s \n", *(result+ack_data));
		printf("[HP]stop result: %04X \n", ack_data);
	}
}
void basic_test_hp_stop(void)
{
	cmd_mission_hp_request_stop	send_data ={0};
	App_Send_Data(2, 1, MY_CTRL_CMD_SET, API_MISSION_HP_STOP, (uint8_t*)&send_data, sizeof(send_data), basic_test_hp_stop_ack_callback, 1000, 0);
	printf("[HP] send HP stop");	
}

void basic_test_hp_pause_ack_callback(ProHeader *header)
{
	uint16_t ack_data;
	printf("Sdk_ack_cmd0_callback,sequence_number=%d,session_id=%d,data_len=%d\n", header->sequence_number, header->session_id, header->length - EXC_DATA_SIZE);
	memcpy((uint8_t *)&ack_data,(uint8_t *)&header->magic, (header->length - EXC_DATA_SIZE));
...
void basic_test_hp_pause(void)
{
	static cmd_mission_hp_request_pause send_data = {0};
	App_Send_Data(2, 1, MY_CTRL_CMD_SET, API_MISSION_HP_PAUSE, (uint8_t*)&send_data, sizeof(send_data), basic_test_hp_pause_ack_callback, 1000, 0);
	printf("[HP] send HP pause msg %d \n", send_data.data);	
...
void att_euler_sp_cb(const dji_sdk::acc&msg)
{

    api_ctrl_without_sensor_data_t send_data = {0};

    send_data.ctrl_flag 	= 0; // moIi
    send_data.roll_or_x 	= msg.ax ;
    send_data.pitch_or_y 	= msg.ay ;
    send_data.thr_z 	= msg.vz; //m/s
...
void att_quad_sp_cb(const dji_sdk::attitude_quad&msg)
{
...
void gimbal_sp_cb(const dji_sdk::gimbal gimbal)
{
    ...
    send_data.yaw_angle   = floor(gimbal.yaw * 10.0f);  // unit 0.1 degree
...
void local_pos_sp_cb(const dji_sdk::local_position lo)
{
    ...
    send_data.ctrl_flag 	= 0x98; // mode 4
...
void spin_callback(const ros::TimerEvent& e)
{
	...
	if(simple_task_num > -1)
	{
		...
		switch(simple_task_num)
		{
			...
			break;
			case 7:
			basic_test_mode10(init_flag);
			break;
			case 8:
			basic_test_gimbal_angle(init_flag);
			break;
			case 9:
			basic_test_gimbal_speed(init_flag);
			break;
			case 10:
			basic_test_hp_start();
			simple_task_num = -1;
...
//    att_sp_sub = nh.subscribe("/attitude_quad_setpoint",10,att_quad_sp_cb);
    acc_sp_sub = nh.subscribe("/acc_setpoint",10,att_euler_sp_cb);
