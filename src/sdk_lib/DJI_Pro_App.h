...
typedef struct __attribute__((packed))
{
	...
	fp32	thr_z;
	fp32	yaw;
}api_ctrl_without_sensor_packed_data_t;

// for gimbal
typedef struct __attribute__((packed))
{
	int16_t	yaw_anglar_rate;
	int16_t	roll_anglar_rate;
	int16_t	pitch_anglar_rate;
...
typedef struct __attribute__((packed))
{
	...
	uint8_t	start_point;	// 0 - north 
				// 1 - south
				// 2 - west
				// 3 - east
				// 4 - cur pos to nearest
	uint8_t	yaw_mode;	// 0 - auto
				// 1 - align to hot point inside
				// 2 - align to hot point outside
				// 3 - user yaw 
				// 4 - no yaw
...
typedef struct __attribute__((packed))
{
	hotpoint_mission_setting_t setting;
}cmd_mission_hp_request_start;

typedef struct __attribute__((packed))
{
	uint8_t data;
}cmd_mission_hp_request_stop;				

typedef struct __attribute__((packed))
{
	uint8_t data;
}cmd_mission_hp_request_pause;				
